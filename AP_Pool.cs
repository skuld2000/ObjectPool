...
namespace AutoPool
{
    public class AP_Pool : MonoBehaviour
    {
        ...
        public class PoolBlock
        {
            ...
            public int maxSize = 50; // absolut limit on pool size, used with EmptyBehavior Grow mode
        ...
        { // use to call spawn directly from the pool, and also used by the "Spawn" button in the editor
            return Spawn(null, null, Vector3.zero, Quaternion.identity, false);
        }
        public GameObject Spawn(Transform parent)
        { // use to call spawn directly from the pool, and also used by the "Spawn" button in the editor
            return Spawn(parent, null, Vector3.zero, Quaternion.identity, false);
        }
        public GameObject Spawn(Transform parent, int? child)
        { // use to call spawn directly from the pool
            return Spawn(parent, child, Vector3.zero, Quaternion.identity, false);
        }
        public GameObject Spawn(Transform parent, Vector3 pos, Quaternion rot)
        { // use to call spawn directly from the pool
            return Spawn(parent, null, pos, rot, true);
        }
        public GameObject Spawn(Transform parent, int? child, Vector3 pos, Quaternion rot)
        { // use to call spawn directly from the pool
            return Spawn(parent, child, pos, rot, true);
        }
        public GameObject Spawn(Transform parent, int? child, Vector3 pos, Quaternion rot, bool usePosRot)
        {
